...
# очко; Д, К, Л, М, П, У – 2 очка; Б, Г, Ё, Ь, Я – 3 очка; Й, Ы – 4 очка; Ж, З, Х, Ц, Ч – 5 очков;
...
if word[0] in 'AEIOULNSTRDGBCMPFHVWYKJXQZ':
    for sym in word:
        for j in points_eng:
            if sym in points_eng[j]:
                points += j
else:
    for sym in word:
        for j in points_rus:
            if sym in points_rus[j]:
                points += j
print(f"Стоимость введенного слова {word}: {points}")